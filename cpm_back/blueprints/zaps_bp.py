"""
Запросы на отгул (zap): создать, список по студенту, все, по ID, обработать, повтор даты.
"""
import base64
...
def create(current_user=None):
    ...
    if not dates:
        return jsonify({'status': False, 'error': 'dates обязателен (минимум одна дата)'}), 400
    try:
    ...
    result = create_zap(student_id, text, dates, images_data if images_data else None)
...
def all_zaps(current_user=None):
    ...
    result = get_all_zaps(status, page=page, limit=limit)
    return jsonify(result), 200 if result.get('status') else 400
...
def process(current_user=None):
    ...
    result = process_zap(zap_id, status, answer)
    return jsonify(result), 200 if result.get('status') else 400


@zaps_bp.route('/zap-dates/<int:zap_date_id>/retry', methods=['POST'])
@require_role('admin')
