...
from cpm_back.services.card_transform.build_preview import IMPORT_TYPE
from cpm_back.services.card_transform.sessions import (
    create_session,
    get_session,
...
card_transform_bp = Blueprint("card_transform", __name__, url_prefix="/api/card-transform")
...
@card_transform_bp.route("/sessions", methods=["POST"])
@require_role("admin")
def sessions_create(current_user=None):
    data = request.get_json(silent=True) or {}
    answer = create_session(
        data,
        created_by=current_user.get("id") if current_user else None,
        created_by_name=current_user.get("full_name") if current_user else None,
    )
    return _schema_status(answer, ok=201)


@card_transform_bp.route("/sessions/<int:session_id>", methods=["GET"])
@require_role("admin")
...
@card_transform_bp.route("/sessions/<int:session_id>/commit", methods=["POST"])
@require_role("admin")
def session_commit(session_id, current_user=None):
    ...
    try:
        ...
        return jsonify(
            {
                "status": True,
                "message": "Трансформация в драфт запущена",
                "job": job,
            }
        )
