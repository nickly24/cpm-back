def queue_and_delete_submission_data(cursor, *, homework_id=None, student_id=None):
    ...
    cursor.execute('INSERT IGNORE INTO homework_s3_delete_queue (object_key) SELECT f.object_key FROM homework_submission_files f JOIN homework_submissions sub ON sub.id=f.submission_id WHERE '+where,tuple(params))
