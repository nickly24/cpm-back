"""Preview-сессии импорта карточек."""
...
from .build_preview import (
    ...
    rebuild_preview_from_cards,
    validate_preview_for_commit,
)
from .parse_excel import parse_cards_excel
...
def _parse_direction_id(value: Any) -> Optional[int]:
    try:
        return int(value)
...
def update_session(session_id: int, preview: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        rebuilt = rebuild_preview_from_cards(cursor, preview)
        cursor.execute(
            """
