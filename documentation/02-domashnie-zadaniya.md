...
> Ниже описан прежний журнал ручной сдачи. Текущая сдача PDF студентом и проверка
> реализованы в отдельном `cpm_homework_service` с общей MySQL; актуальный контракт
> находится в [README файлового сервиса](../../cpm_homework_service/README.md).
> Основной API возвращает `submission_state`, `submission_id`, `has_file`, `has_draft`,
> `submitted_at_utc`, `revision_comment` и `file_managed` в списках. Старый `status`
> по-прежнему отражает итоговую оценку. Ручные pass/edit/reset для файловой сдачи
> отклоняются с `409 use_file_review`; нужно пользоваться очередью или архивом.
> Проктор ограничен учениками своей текущей группы и не может подставить чужой
> `proctorId`. `/get-homeworks` требует авторизации. Без миграции файлового сервиса
> старый журнал продолжает работать, новые поля имеют пустые значения.
